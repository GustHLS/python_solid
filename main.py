from cliente import Cliente
from item import Item
from pedido.pedido_facade import PedidoFacade
from pagamento.pagamento_factory import PagamentoFactory
from notificacao.notificacao_facade import NotificacaoFacade
from observador.observador_status import ObservadorStatus

cliente = Cliente("Gustavo", "Rua Alegre, 123")
item_1 = Item("Camiseta", 29.99)
item_2 = Item("Calça Jeans", 49.99)
itens = [item_1, item_2]

tipo_pedido = "delivery" # retirada ou delivery
taxa_entrega = 10.0
pedido = PedidoFacade.criar_pedido_retirada(tipo_pedido, cliente, itens, taxa_entrega)
valor_pedido = pedido.calcular_total()

tipo_pagamento = "pix"
pagamento = PagamentoFactory().criar_pagamento(tipo_pagamento)
pagamento.processar(valor_pedido)

MENSAGEM_PAGO = "Pagamento foi confirmado!"
MENSAGEM_PREPARANDO = "O pedido está sendo preparado!"
MENSAGEM_ENVIADO = "O pedido saiu para entrega!"

notificacoes = NotificacaoFacade()
observador = ObservadorStatus(notificacoes)
pedido.adicionar_observadoradores(observador)

pedido.status = MENSAGEM_PAGO
pedido.status = MENSAGEM_PREPARANDO
pedido.status = MENSAGEM_ENVIADO